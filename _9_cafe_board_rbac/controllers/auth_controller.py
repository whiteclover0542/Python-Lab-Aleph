...
핵심 규칙: 회원가입은 항상 옵저버(role=0)로만 생성된다. 요청 바디에 role 을
몰래 끼워 보내도(예: {"role": 2}) 서버는 그 값을 절대 읽지 않는다 — 등급 상승은
오직 센티널 콘솔(/api/admin/users)을 통해서만 가능해야 하기 때문이다.
"""
...
def register():
  ...
  # 클라이언트가 role 을 보내더라도 무시하고 항상 옵저버로 가입시킨다.
  user = User(username=username, password=generate_password_hash(password),
              role=ROLE_GENERAL)
  db.session.add(user)
  db.session.commit()
  return jsonify({'msg': '회원가입 성공 (옵저버로 가입되었습니다.)'}), 201
...
def me():
  ...
  센티널이 방금 등급을 바꿨어도 이 엔드포인트를 통해 재로그인 없이 최신 등급을
  바로 확인할 수 있다 — 화면(헤더 배지, 가디언/센티널 페이지 가드)이 이 응답으로 갱신된다.
  """
